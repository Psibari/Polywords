...
Daily Polly (SHIPPED — pose images):

* Daily perch (`PollyDailyPerch`) uses clean full-pose drawings from `assets/images/polly/poses/*.png` (background stripped to transparent). NOT the rig — the rig's rough cut showed seams/double-beak, so it's shelved until proper layered art exists.
* Poses: `sprite4` smug idle · `sprite7` point (wrong) · `sprite5` laugh (loss) · `sprite8` shocked (win) · `sprite2` fly-in entrance. Reaction swaps pose + whole-image motion (lean/shake/pop) + laugh SFX (`pollySqwawkShort` wrong, `pollySqwawkLaugh` out-of-lives). She's bottom-left, faces right, branch rooted off the left edge, breath+sway idle, perched continuously (no per-round pop).
* Source sprites (purple bg) live in `assets/images/sprite1-10.png`; re-strip via the pngjs flood-fill script if re-exported.

Living rig (shelved for future layered art):

* `PollyRig` 13-part puppet + declarative choreography in `pollyPerformances.ts` (idle/smug/laugh/shocked). Kept in-repo; swap Daily back to it when proper full-layer art (each part drawn complete on a 512 canvas) replaces the rough cut. Spec/plan: `docs/superpowers/{specs,plans}/2026-07-02-polly-*`.
