...
5. Find the human memory handle for each approved meaning.
6. Draft recognition-first REALS from those handles.
7. Build traps independently from semantic neighbors.
8. Run the complete audit and obtain word approval.

Definitions establish legality; human handles make the game playable. Prefer controlled
five-word review batches until quality is stable.
...
- Avoid repeating key nouns, settings, and semantic scenery across one word’s tiles.

## REALS

A REAL represents exactly one distinct, sourced meaning. It must be legally true, familiar
after reveal, natural, and indirect enough to make the brain reach.
...
- Draft from an independent neighbor pool, not one mirrored trap per REAL.
...
8. Every REAL maps to one distinct sourced meaning?
9. Every trap is tempting but legally wrong?
10. Could no fair player defend the trap as a real meaning?
11. REAL/trap wording has Hidden Truth parity?
12. No repetitive semantic scenery?
13. At least one meaning creates a meaningful context switch—the Semantic Snap?
